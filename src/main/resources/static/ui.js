...
export function renderDashboard() {
    ...
    tbody.innerHTML = recent.length ? recent.map(txRowFull).join('') : emptyRow(6);
}
...
export function renderAllTx() {
    const selectedAccountId = $('tx-acc-filter')?.value || '';
    const filtered = selectedAccountId
        ? state.transactions.filter(t => {
            const acc = String(selectedAccountId);
            return String(state.accounts.find(a => a.accountName === t.fromAccount)?.id || '') === acc
                || String(state.accounts.find(a => a.accountName === t.toAccount)?.id || '') === acc;
        })
        : state.transactions;

    const tbody = $('all-tx-body');
    tbody.innerHTML = filtered.length ? filtered.map(txRowFull).join('') : emptyRow(6, 'No transactions found for this filter');
}

window.renderAllTx = renderAllTx;
